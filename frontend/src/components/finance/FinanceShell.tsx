import { BarChart3, CalendarClock, LayoutDashboard, Moon, Plus, Receipt, Sun, Target, WalletCards } from "lucide-react";
    ...
    <div className="min-h-screen bg-background text-foreground" data-testid="cash-app">
      ...
      <div className="relative grid min-h-screen lg:grid-cols-[256px_1fr]">
        <aside className="hidden border-r border-border/70 bg-card/55 px-4 py-6 backdrop-blur-xl lg:flex lg:flex-col" data-testid="desktop-sidebar">
          ...
          <div className="mt-auto rounded-2xl border border-border/70 bg-background/70 p-4" data-testid="sidebar-goal-card"><div className="flex items-center justify-between"><span className="flex size-8 items-center justify-center rounded-lg bg-primary/10 text-primary"><Target className="size-4" /></span><span className="font-mono text-[10px] uppercase tracking-widest text-muted-foreground" data-testid="sidebar-goal-kicker">Foco</span></div><p className="mt-4 text-sm font-medium" data-testid="sidebar-goal-title">Um mês de cada vez</p><p className="mt-1 text-xs leading-5 text-muted-foreground" data-testid="sidebar-goal-description">Seu dinheiro fica mais claro quando o próximo passo também fica.</p></div>
        </aside>
